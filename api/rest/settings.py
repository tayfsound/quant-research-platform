...
def _validate(key: str, value: str) -> None:
    ...
    elif key == "direction_trading_enabled":
        # Kullanıcı isteği (2026-08-28) — regime_trading_enabled ile AYNI
        # desen, JSON dict {"LONG": true, "SHORT": true}.
        import json as _json
        try:
            mapping = _json.loads(value)
            if not isinstance(mapping, dict):
                raise ValueError
            if not set(mapping.keys()) <= {"LONG", "SHORT"}:
                raise ValueError
            for v in mapping.values():
                if not isinstance(v, bool):
                    raise ValueError
            ...
            raise HTTPException(
                400, "direction_trading_enabled must be a JSON object of {'LONG'|'SHORT': true|false}"
            )
    ...
    # multi_timeframe_cascade_enabled / _timeframes doğrulamaları Faz
    # 457'de kaldırıldı — cascade mimariden çıktı, ayarların kendisi de
    # DEFAULT_SETTINGS'ten silindi (aşağıdaki genel "bilinmeyen anahtar"
    # kontrolü artık bunları zaten reddediyor).
    ...
    elif key == "agent_combination_gate_min_win_rate":
        try:
            v = float(value)
            if not (0 < v <= 1):
                raise ValueError
        except ValueError:
            raise HTTPException(400, "agent_combination_gate_min_win_rate must be a number in (0, 1]")

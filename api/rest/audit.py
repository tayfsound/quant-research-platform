...
async def get_trade_audit(trade_id: UUID, user: AuthContext = Depends(get_current_user)):
    return {"trade_id": str(trade_id), "status": "Audit service active (stub)"}

@router.get("/trades")
async def list_trades(symbol: str = "BTCUSDT", limit: int = 100, user: AuthContext = Depends(get_current_user)):

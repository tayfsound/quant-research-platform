...
| 9 | **İkinci, tamamen kopuk bir Replay motoru var.** `services/replay/` (replay_store, replay_session, seed_manager, decision_hash, state_restorer, event_applier, replay_verifier, snapshot_manager, event_stream, snapshot_builder — 11 dosya, ~280 satır) + `engines/replay/replay_engine.py` (`DeterministicReplayEngine`) sadece kendi test paketinden (`tests/replay/`, 12 test, hepsi yeşil) çağrılıyor; API, dashboard, orchestrator hiçbiri bunu kullanmıyor. Gerçek üretim yolu `services/replay_engine.py` (`ReplayEngine` — Faz 162/164/166'nın üzerine inşa edildiği, `api/rest/replay.py`'a bağlı, gerçek DB'ye karşı test edilmiş olan). Roadmap'in Sprint 2 maddesi ("`services/replay/` → `orchestrator.py` bağlantısı") tam olarak bunu işaret ediyor. **Bilinçli olarak bugün bağlanmadı**: hangi motorun kalacağına (birleştir / `services/replay/`'ı kaldır / iki ayrı kullanım alanı tanımla) karar vermeden zorla bağlamak "iki beyin" ihlalini replay tarafında da açardı — bu proje sahibinin kararı gerektiriyor. | P1 | Sprint 2 gate |

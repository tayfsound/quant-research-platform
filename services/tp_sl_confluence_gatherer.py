...
def _analyze_symbol(provider: RoutingProvider, symbol: str, stop_mult: float, target_mult: float, min_stop_pct: float) -> dict | None:
    try:
    ...
    # Yön agnostik ölçüm — RiskTargetStage yöne göre iki farklı fiyat
    # üretir (LONG: stop aşağı/target yukarı, SHORT: tersi), ikisi de
    # ayrı ayrı kontrol ediliyor.
    long_stop = current_price * (1 - stop_pct)
    long_target = current_price * (1 + target_pct)
    short_stop = current_price * (1 + stop_pct)
    short_target = current_price * (1 - target_pct)
...
def gather_tp_sl_confluence() -> dict:
    ...
    with SessionFactory.get_session() as session:
        ...
        stop_mult = float(repo.get("stop_atr_mult") or DEFAULT_STOP_ATR_MULT)
        target_mult = float(repo.get("target_atr_mult") or DEFAULT_TARGET_ATR_MULT)
    ...
    for symbol in symbols:
        result = _analyze_symbol(provider, symbol, stop_mult, target_mult, min_stop_pct)

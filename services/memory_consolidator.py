...
class MemoryConsolidator:
    ...
    def capture_cycle(self, ctx):
        self.episodic.episodes.append(Episode(
            symbol=ctx.market.get("symbol", ""),
            binding_expression="",
            decision=ctx.decision.get("proposed_direction", "WAIT"),
            outcome=ctx.outcome,
            lesson="",
        ))
        for k in ctx.cognition.relevant_knowledge:
            if k.get("type") == "observation":
                self.semantic.observations.append(Observation(
                    type=ObservationType.INDICATOR,
                    symbol=ctx.market.get("symbol", ""),
                    timeframe=ctx.market.get("timeframe", ""),
                    description=k.get("data", {}).get("description", ""),
                    data=k.get("data", {}),
                ))

    def commit_to_episodic(self, ctx):
        repo = EpisodeRepository(ctx.session)
        for ep in self.episodic.episodes:
            repo.save(ep)

    def load_from_persistent(self, session):
        with session:
    ...
    def memory_stats(self):
        return {
            ...
            "observations_in_ram": len(self.semantic.observations),

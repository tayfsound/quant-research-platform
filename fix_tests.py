...
# 1. test_llm_explainer.py -- subprocess -> httpx
with open('tests/contract/test_llm_explainer.py', 'r') as f:
    content = f.read()

new_content = '''"""OllamaExplainer contract testleri."""
import asyncio
...
with open('tests/contract/test_llm_explainer.py', 'w') as f:
    f.write(new_content)
print('✓ tests/contract/test_llm_explainer.py httpx uyumlu')

# 2. test_integration_cycle.py -- daha basit test
...
''')
print('✓ tests/test_integration_cycle.py duzeltildi')
